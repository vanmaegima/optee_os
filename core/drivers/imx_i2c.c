...
/* Clock: i.MX 8M APRM Reference Manual Rev2, 08/2019, page 5281 */
...
static void i2c_set_prescaler(uint8_t bid, uint32_t rate)
{
	struct ifdr_pair *p = ifdr_table;
	struct ifdr_pair *q = p + ARRAY_SIZE(ifdr_table) - 1;
	uint32_t div = (I2C_CLK_RATE + rate - 1) / rate;
...
static void enable_i2c_clk(uint8_t bid)
{
	struct imx_i2c_clk *c = &i2c_clk;

	io_write32(c->base.va + CCM_CCGRx_SET(c->i2c[bid]), CCGR_CLK_ON_MASK);
}

static void i2c_set_bus_speed(uint8_t bid, int speed)
{
	enable_i2c_clk(bid);
	i2c_set_prescaler(bid, speed);
}
...
static TEE_Result i2c_read_data(uint8_t bid, uint8_t *buf, int len)
{
	...
	if (ret != TEE_SUCCESS)
		return ret;
	/*
	 * i.MX 8M Mini APRM Rev2 08/2019:
...
/* i.MX 8M Mini APRM Rev2 08/2019: steps previous to a read/write operation  */
static TEE_Result i2c_init_transfer(uint8_t bid, uint8_t chip)
{
	...
	/* 16.1.4.1 start master transaction */
	tmp = i2c_io_read8(bid, I2CR) | I2CR_MSTA;
	i2c_io_write8(bid, I2CR, tmp);

	/* 16.1.4.2. on multimaster bus system, wait unil we have the bus */
	ret = i2c_sync_bus(bid, &bus_is_busy, NULL);
	if (ret != TEE_SUCCESS)
		return ret;

	/* put the slave address on the bus */
...
TEE_Result imx_i2c_read(uint8_t bid, uint8_t chip, uint8_t *buf, int len)
{
	...
	if (bid > ARRAY_SIZE(i2c_bus) - 1)
		return TEE_ERROR_BAD_PARAMETERS;

	if ((len && !buf) || chip > 0x7F)
		return TEE_ERROR_BAD_PARAMETERS;

	ret = i2c_init_transfer(bid, chip << 1 | 1);
	if (ret == TEE_SUCCESS)
...
TEE_Result imx_i2c_init(uint8_t bid, int rate)
{
	...
	/* 16.1.4.1 set the data sampling rate */
	i2c_set_bus_speed(bid, rate);
...
static TEE_Result i2c_early_init(void)
{
	size_t n = 0;
...
early_driver_init(i2c_early_init);
