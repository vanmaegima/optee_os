...
static sss_status_t set_ecc_public(Se05xSession_t *s_ctx,
				   ...
				   struct se050_ecc_keypub *keypub)
{
	size_t pubkey_len = keypub->x_len + keypub->y_len + 1;
				    ...
				    struct se050_ecc_keypair *keypair)
{
	...
	k_object->curve_id = keypair->curve;

	status = Se05x_API_WriteECKey(s_ctx, policy, SE05x_MaxAttemps_UNLIMITED,
				      k_object->keyId,
				      (SE05x_ECCurve_t)keypair->curve,
				      keypair->d,
				      keypair->d_len,
				      NULL,
				      0,
...
static sss_status_t set_ecc_pair(Se05xSession_t *s_ctx,
				 ...
				 struct se050_ecc_keypair *keypair)
{
	size_t pubkey_len = keypair->x_len + keypair->y_len + 1;
	smStatus_t status = SM_NOT_OK;
	uint8_t *pubkey = NULL;
	uint8_t *buf = NULL;
	...
	*buf = 0x04;
	pubkey = buf + 1;
	memcpy(pubkey, keypair->x, keypair->x_len);
	memcpy(pubkey + keypair->x_len, keypair->y, keypair->y_len);
