// SPDX-License-Identifier: BSD-2-Clause

#include "aes.h"

static const uint8_t sbox[256] = {
	,
};

static const uint8_t inv_sbox[256] = {
	
};

static const uint8_t rcon[255] = {
	
};

static void rotate(uint8_t *word)
{
	uint8_t c = word[0];

	word[0] = word[1];
	word[1] = word[2];
	word[2] = word[3];
	word[3] = c;
}

static void key_expansion_core(uint8_t *word, int iteration)
{
	size_t i = 0;

	rotate(word);

	for (i = 0; i < 4; ++i)
		word[i] = sbox[word[i]];

	word[0] ^= rcon[iteration];
}

static void key_expansion(struct aes_ctx *ctx, int len, int new_len)
{
	uint8_t t[4] = { 0 };
	int iteration = 1;
	size_t i = 0;

	while (len < new_len) {
		for (i = 0; i < 4; i++)
			t[i] = ctx->round_key[len - 4 + i];

		if (len % ctx->key_len == 0)
			key_expansion_core(t, iteration++);

		if (ctx->key_len == 32 && ((len % ctx->key_len) == 16)) {
			for (i = 0; i < 4; i++)
				t[i] = sbox[t[i]];
		}

		for (i = 0; i < 4; i++, len++) {
			ctx->round_key[len] =
				ctx->round_key[len - ctx->key_len] ^ t[i];
		}
	}
}

static void sub_bytes(struct aes_ctx *ctx)
{
	size_t i = 0, j = 0;

	for (i = 0; i < 4; i++)
		for (j = 0; j < 4; j++)
			ctx->state[i][j] = sbox[ctx->state[i][j]];
}

static void inv_sub_bytes(struct aes_ctx *ctx)
{
	size_t i = 0, j = 0;

	for (i = 0; i < 4; i++)
		for (j = 0; j < 4; j++)
			ctx->state[i][j] = inv_sbox[ctx->state[i][j]];
}

static void shift_rows(struct aes_ctx *ctx)
{
	uint8_t new_state[4][4];
	size_t i = 0, j = 0;

	for (i = 0; i < 4; i++)
		for (j = 0; j < 4; j++)
			new_state[i][j] = ctx->state[i][(i + j) % 4];

	memcpy(ctx->state, new_state, sizeof(ctx->state));
}

static void inv_shift_rows(struct aes_ctx *ctx)
{
	uint8_t new_state[4][4];
	size_t i = 0, j = 0;

	for (i = 0; i < 4; i++)
		for (j = 0; j < 4; j++)
			new_state[i][j] = ctx->state[i][(j - i + 4) % 4];

	memcpy(ctx->state, new_state, sizeof(ctx->state));
}

#define xtime(x, y) (((x) << 1) ^ ((((y) >> 7) & 1) * 0x1b))

static uint8_t multiply(uint8_t a, uint8_t b)
{
	size_t i = 0;
	uint8_t c = 0;
	uint8_t d = b;

	for (i = 0; i < 8; i++) {
		if (a % 2 == 1)
			c ^= d;
		a /= 2;
		d = xtime(d, d);
	}

	return c;
}

static void mix_columns(struct aes_ctx *ctx)
{
	uint8_t new_state[4][4];
	size_t i = 0;

	for (i = 0; i < 4; i++) {
		new_state[0][i] = multiply(2, ctx->state[0][i]) ^
				  multiply(3, ctx->state[1][i]) ^
				  ctx->state[2][i] ^
				  ctx->state[3][i];

		new_state[1][i] = ctx->state[0][i] ^
				  multiply(2, ctx->state[1][i]) ^
				  multiply(3, ctx->state[2][i]) ^
				  ctx->state[3][i];

		new_state[2][i] = ctx->state[0][i] ^
				  ctx->state[1][i] ^
				  multiply(2, ctx->state[2][i]) ^
				  multiply(3, ctx->state[3][i]);

		new_state[3][i] = multiply(3, ctx->state[0][i]) ^
				  ctx->state[1][i] ^
				  ctx->state[2][i] ^
				  multiply(2, ctx->state[3][i]);
	}

	memcpy(ctx->state, new_state, sizeof(ctx->state));
}

static void inv_mix_columns(struct aes_ctx *ctx)
{
	uint8_t new_state[4][4];
	size_t i = 0;

	for (i = 0; i < 4; i++) {
		new_state[0][i] = multiply(0x0e, ctx->state[0][i]) ^
				  multiply(0x0b, ctx->state[1][i]) ^
				  multiply(0x0d, ctx->state[2][i]) ^
				  multiply(0x09, ctx->state[3][i]);

		new_state[1][i] = multiply(0x09, ctx->state[0][i]) ^
				  multiply(0x0e, ctx->state[1][i]) ^
				  multiply(0x0b, ctx->state[2][i]) ^
				  multiply(0x0d, ctx->state[3][i]);

		new_state[2][i] = multiply(0x0d, ctx->state[0][i]) ^
				  multiply(0x09, ctx->state[1][i]) ^
				  multiply(0x0e, ctx->state[2][i]) ^
				  multiply(0x0b, ctx->state[3][i]);

		new_state[3][i] = multiply(0x0b, ctx->state[0][i]) ^
				  multiply(0x0d, ctx->state[1][i]) ^
				  multiply(0x09, ctx->state[2][i]) ^
				  multiply(0x0e, ctx->state[3][i]);
	}

	memcpy(ctx->state, new_state, sizeof(ctx->state));
}

static void add_roundkey(struct aes_ctx *ctx, int round)
{
	size_t i = 0, j = 0;

	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			ctx->state[i][j] ^=
			ctx->round_key[round * AES_BLOCKSIZE + j * 4 + i];
		}
	}
}

void AES_encrypt(struct aes_ctx *ctx, uint8_t *in, uint8_t *out)
{
	int round = 0;
	size_t i = 0;

	for (i = 0; i < AES_BLOCKSIZE; i++)
		ctx->state[i % 4][i / 4] = in[i];

	add_roundkey(ctx, 0);

	for (round = 1; round <= ctx->rounds; round++) {
		sub_bytes(ctx);
		shift_rows(ctx);
		if (round != ctx->rounds)
			mix_columns(ctx);
		add_roundkey(ctx, round);
	}

	for (i = 0; i < AES_BLOCKSIZE; i++)
		out[i] = ctx->state[i % 4][i / 4];
}

void AES_decrypt(struct aes_ctx *ctx, uint8_t *in, uint8_t *out)
{
	int round = 0;
	size_t i = 0;

	for (i = 0; i < AES_BLOCKSIZE; i++)
		ctx->state[i % 4][i / 4] = in[i];

	add_roundkey(ctx, ctx->rounds);

	for (round = ctx->rounds - 1; round >= 0; round--) {
		inv_shift_rows(ctx);
		inv_sub_bytes(ctx);
		add_roundkey(ctx, round);
		if (round)
			inv_mix_columns(ctx);
	}

	for (i = 0; i < AES_BLOCKSIZE; i++)
		out[i] = ctx->state[i % 4][i / 4];
}

struct aes_ctx *AES_ctx_alloc(uint8_t *key, size_t key_len)
{
	struct aes_ctx *ctx = NULL;
	size_t key_size = 0;
	int rounds = 0;

	switch (key_len) {
	case 16:
		rounds = 10; /* AES128 */
		break;
	case 24:
		rounds = 12; /* AES192 */
		break;
	case 32:
		rounds = 14; /* AES256 */
		break;
	default:
		return NULL;
	}

	key_size = (rounds + 1) * AES_BLOCKSIZE;

	ctx = malloc(sizeof(struct aes_ctx));
	if (!ctx)
		return NULL;

	ctx->rounds = rounds;
	ctx->key_len = key_len;
	memcpy(ctx->round_key, key, key_len);
	key_expansion(ctx, key_len, key_size);

	return ctx;
}
